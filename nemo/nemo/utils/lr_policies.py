...
class _LRPolicy(ABC):
    ...
    @abstractmethod
    def __call__(self, initial_lr: float, step: int, epoch: int) -> float:
...
class WarmupPolicy(_LRPolicy):
    ...
    def _get_lr(self, initial_lr: float, step: int, epoch: int) -> float:
        """Default policy: constant lr"""
        return initial_lr


def _square_annealing(initial_lr: float,
                      step: int,
                      total_steps: int,
                      min_lr: float) -> float:
...
def _cosine_annealing(initial_lr: float, step: int, total_steps: int) -> float:
...
def get_all_lr_classes() -> Dict[str, Callable[WarmupPolicy]]:
...
def get_lr_policy_fn(lr_policy: str, **kwargs) -> WarmupPolicy:
